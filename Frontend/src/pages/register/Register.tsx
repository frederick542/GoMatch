...
export default function Register({navigation: {navigate}}: Props) {
  ...
  const {executeAsync: handleSendEmailOTP} = useAsyncHandler(async function () {
  ...
  const {executeAsync: handleRegister} = useAsyncHandler(async function () {
        ...
        <View style={styles.otpDiv}>
          {[0, 1, 2, 3].map(idx => {
            return (
              <OtpPlaceholder
                code={otp[idx]}
                openInput={openOtpInput}
                key={`OtpPlaceHolder${idx}`}
              />
            );
          })}
        </View>
    ...
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={handlePickImage}>
        <Image
          ...
          source={renderProfileImage(profileUri)}
        />
      </TouchableOpacity>
      <TextInput
        style={styles.input}
